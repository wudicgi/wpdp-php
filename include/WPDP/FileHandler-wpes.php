...
class WPDP_FileHandler {
    ...
    private $_data_size = 4096;
    private $_head_size = 32;
    private $_block_size = 4128; // 4096 + 32
    ...
    function open($filename, $mode) {
        ...
        assert('$file_length % $this->_block_size == 0');
        $num_blocks = (int)($file_length / $this->_block_size);
        trace(__METHOD__, ((string)$this->_fp) . ": " . "file_length = " . $file_length . ", num_blocks = " . $num_blocks);
        if ($num_blocks > 0) {
            $offset_last_block = $file_length - $this->_block_size;
            $this->_readBuffer($offset_last_block);
            trace(__METHOD__, ((string)$this->_fp) . ": " . "set file length to (" . $this->_data_size . " * (" . $num_blocks . " - 1)) + " . $this->_buffer_length . " = " . (($this->_data_size * ($num_blocks - 1)) + $this->_buffer_length));
            $this->_length = ($this->_data_size * ($num_blocks - 1)) + $this->_buffer_length;
        } else {
            $this->_length = 0;
        }
//        echo "this->_length = " . $this->_length . "<br >\n";
    ...
    private function _getBlockOffset($offset) {
        $block_offset = ((int)($offset / $this->_data_size)) * $this->_block_size;
        return $block_offset;
    }
    ...
    private function _readBuffer($offset_block) {
        ...
        $head = fread($this->_fp, $this->_head_size);
        ...
        $this->_buffer = fread($this->_fp, $this->_data_size);
    ...
    private function _writeBuffer() {
        ...
        if (strlen($this->_buffer) < $this->_data_size) {
            $this->_buffer .= str_repeat("\x00", $this->_data_size - strlen($this->_buffer));
        }
        ...
        $data .= pack('v', $this->_data_size); // data_size
...
//        echo "_writeBuffer(): " . $this->_buffer_offset . ", " . ftell($this->_fp) . ", " . strlen($data) ." bytes<br />\n";
    ...
    function read($length) {
        ...
        while ($didread < $length) {
            ...
            $len_ahead = $this->_offset % $this->_data_size;
...
//            echo "has read $didread of $length, $len_ahead, $len_behind, $len_read<br />\n";
            if ($len_ahead == 0 && $len_read == $this->_data_size) { // notice, diff to write()
                $data .= $this->_buffer;
            } else {
    ...
    function write($data) {
        ...
        while ($didwrite < $length) {
            ...
            $len_ahead = $this->_offset % $this->_data_size;
            // 计算在块中，当前位置后数据的长度
            $len_behind = $this->_data_size - $len_ahead;
            // 计算在块中，可写数据的长度
